...
async def lifespan(app: FastAPI):
    ...
    if not settings.server.debug:
        ...
        if os.getenv("TRINITY_MODE") == "microservice":
            # ============== 🏛️ 三脑分立架构 ==============
            # Brain (9000), Mouth (9001), Ear (9002) 完全独立
            logger.info("🏛️ 三脑分立架构: 连接到 Cortex 服务集群...")
            ...
            # 1. Remote Brain (端口 9000)
        try:
                logger.info(f"🧠 连接 Brain -> {brain_url}")
                brain_adapter = BrainAdapter(
                    model_path="REMOTE",
                    remote_url=brain_url
                )
                await brain_adapter.initialize()
                logger.success("✓ Remote BrainAdapter 就绪")
            except Exception as e:
                logger.error(f"✗ Remote BrainAdapter 失败: {e}")

            # 2. Remote Mouth (端口 9001)
            try:
                logger.info(f"👄 连接 Mouth -> {mouth_url}")
                mouth_adapter = MouthAdapter(
                    model_path="REMOTE",
                    remote_url=mouth_url
                )
                await mouth_adapter.initialize()
                logger.success("✓ Remote MouthAdapter 就绪")
            except Exception as e:
                logger.error(f"✗ Remote MouthAdapter 失败: {e}")
                
            # 3. Remote Ear (端口 9002) - ASR 也走远程
            try:
                logger.info(f"👂 连接 Ear -> {ear_url}")
                # VoiceAdapter 现在也可以远程连接
                ...
                logger.success("✓ Remote VoiceAdapter 就绪")
        except Exception as e:
                logger.error(f"✗ Remote VoiceAdapter 失败: {e}")
