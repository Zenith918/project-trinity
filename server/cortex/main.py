...
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  🏛️ 新架构 (2026年1月): 三脑分立 (Trinity Cortex Split)                       ║
║                                                                              ║
║  ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐                 ║
║  │ brain_server.py │ │ mouth_server.py │ │ ear_server.py   │                 ║
║  │ 端口 9000       │ │ 端口 9001       │ │ 端口 9002       │                 ║
║  │ 🧠 LLM          │ │ 👄 TTS          │ │ 👂 ASR          │                 ║
║  │ 重启 ~90s       │ │ 重启 ~20s       │ │ 重启 ~5s        │                 ║
║  └─────────────────┘ └─────────────────┘ └─────────────────┘                 ║
║                                                                              ║
║  优势: 每个服务可独立重启，互不干扰！                                           ║
║                                                                              ║
║  🚫 本文件 (main.py) 已弃用，请勿使用                                          ║
║  ✅ 使用: python -m uvicorn server.cortex.brain_server:app --port 9000       ║
║  ✅ 使用: python -m uvicorn server.cortex.mouth_server:app --port 9001       ║
║  ✅ 使用: python -m uvicorn server.cortex.ear_server:app --port 9002         ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
